...
class MLEnricher:
    def __init__(self, contamination_rate=0.001):
        """
        ...
        #processes to ignore
        self.ignored_processes = {'system idle process', 'idle', 'system', 'python.exe', 'python', 'registry', 'msi.centralserver.exe'}
    ...
    def train(self, historical_data):
        ...
        """
        filtered_data = [p for p in historical_data if not self._is_system_or_ignored(p)]
        ...
        X_train = self._feature_engineering(filtered_data)
    ...
    def predict_anomaly(self, live_process):
        ...
        if self._is_system_or_ignored(live_process):
            return False
    ...
    def _is_system_or_ignored(self, live_process):
        """
        Helper to strictly exclude system binaries and known services by PID and Name.
        """
        pid = live_process.get('pid')
        name = str(live_process.get('name', '')).strip().lower()

        # Kernel and Idle processes exclusion by PID
        if pid in (0, 4):
            return True
            
        # Process exclusion by name
    ...
    def save_to_dataset(self, live_process, label=0):
        ...
        """
        # Exclude system and ignored processes right at the entry point
        if self._is_system_or_ignored(live_process):
            return
